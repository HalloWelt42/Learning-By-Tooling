<script>
  ...
  let numSize = $derived(Math.max(7, Math.round(size * 0.14)))
...
<div class="shield" class:tier4={tier === 4} style:--glow="{color.hex}60">
  {#if tier <= 1}
    ...
    <svg width={size} height={h} viewBox="0 0 {size} {h}">
      ...
      {#if canShowNum}
        <text x={size/2} y={h*0.76} text-anchor="middle" font-family="'DM Mono',monospace" font-weight="800" font-size={numSize} fill={color.hex} opacity="0.6">{level}</text>
      {/if}
    </svg>
  ...
  {:else if tier === 2}
    ...
    <svg width={size} height={h} viewBox="0 0 {size} {h}">
      ...
      <path d={innerT1} fill="#0d0d0f"/>
      {#if canShowNum}
        <text x={size/2} y={h*0.76} text-anchor="middle" font-family="'DM Mono',monospace" font-weight="800" font-size={numSize} fill={color.hex}>{level}</text>
      {/if}
    </svg>
  ...
  {:else if tier === 3}
    ...
    <svg width={size} height={h} viewBox="0 0 {size} {h}" style="filter:drop-shadow(0 0 4px {color.hex}40)">
      ...
      {#if canShowNum}
        <text x={size/2} y={h*0.74} text-anchor="middle" font-family="'DM Mono',monospace" font-weight="800" font-size={numSize} fill={light30}>{level}</text>
      {/if}
    </svg>
    <i class="fa-solid {icon} icon-center" style="color:{color.hex};font-size:{iconSize}px"></i>
  ...
  {:else}
    ...
    <svg width={size} height={h} viewBox="0 0 {size} {h}">
      ...
      {#if canShowNum}
        <text x={size/2} y={h*0.74} text-anchor="middle" font-family="'DM Mono',monospace" font-weight="800" font-size={numSize} fill={light40}>{level}</text>
      {/if}
    </svg>
